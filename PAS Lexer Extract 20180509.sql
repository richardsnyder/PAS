	...
	SELECT FactRetailSales.HeaderSourceKey
		...
		,SUM(CASE 
				WHEN RetailLineTypeId = 3
					OR (
						RetailLineTypeId = 2
						AND RetailDiscountReasonId = 330
						)
					THEN FactRetailSales.SalesExcludingTaxLocal
...
SELECT RetailSales.RetailCustomerId AS [customer_id]
	...
	,CASE WHEN Mobile.MobileAppEmail IS NOT NULL THEN 'TRUE' ELSE NULL END AS [Has_App]
FROM RetailSales
...
LEFT OUTER JOIN (
	SELECT [Person Id] PersonId
		,[Apparel 21 Code] Ap21Code
		,[First Name] FirstName
		,[Last Name] LastName
		,[Email Address] Email
		,MobileAppUserId
		,MobileAppThirdPartyCustomerId
		,MobileAppFirstName
		,MobileAppLastName
		,MobileAppEmail
	FROM DataWarehouseUserView.dbo.[Retail Customers] AS RetailCustomers
  LEFT OUTER JOIN (
		    SELECT AMA_Review_Users.id AS MobileAppUserId
			    ,AMA_Review_Users.third_party_customer_id AS MobileAppThirdPartyCustomerId
			    ,AMA_Review_Users.first_name AS MobileAppFirstName
			    ,AMA_Review_Users.last_name AS MobileAppLastName
			    ,AMA_Review_Users.email AS MobileAppEmail
		    FROM DataWarehouseRawData.dbo.AMA_Review_Users
		    WHERE email IS NOT NULL
		    ) AS MobileApp ON MobileApp.MobileAppEmail = RetailCustomers.[Email Address]
	) AS Mobile ON Mobile.PersonId = RetailSales.RetailCustomerId
