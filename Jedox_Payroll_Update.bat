...
if exist PayrollBatch*.log (
  del PayrollBatch*.log
)
...
set LogFileName="%ret%\PayrollBatch-%DateN%_%timen%.log"
...
c:
cd "C:\Program Files\Jedox\Jedox Suite\tomcat\client"
echo ***  Start of Jedox nigtly payroll batch: %Date_now% %Time_now%  ***  >> %LogFileName%
...
IF %DebugFlag% EQU 0 (
  call .\etlclient -p "[09-00]-PAS-FINANCIALS [Payroll Costing]" -j "[00-01-00] Payroll Costing Nightly Update" -c PrevYear=%PrevYear% CurrYear=%CurrYear% ForecastVersion=%FcastVer%>>%LogFileName%
)
...
echo send-mailmessage -from ^"JedoxAdmin@pasco.com.au^" -to ^"RSnyder@pasco.com.au^" ,^"LMcInTyre@pasco.com.au^",^"AYoung@pasco.com.au^" -subject %SubjectText% -body $body -Attachment "%ret%" -priority High -dno onFailure -smtpServer mtwrly01.onepas.local >>%OutFile%
