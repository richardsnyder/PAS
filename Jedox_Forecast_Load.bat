...
if exist ForecastLoadBatch*.log (
  del ForecastLoadBatch*.log
)
...
set LogFileName="%ret%\ForecastLoadBatch-%DateN%_%timen%.log"
...
:: ====== Subroutine Codes ==============
set Forecast_Load_ReturnCode=0
...
c:
cd "C:\Program Files\Jedox\Jedox Suite\tomcat\client"
echo ***  Start of Jedox Retail Weekly Forecast Load Batch: %Date_now% %Time_now%  ***  >> %LogFileName%
...
IF %DebugFlag% EQU 0 (
  call .\etlclient -p "[04-01]-PAS-FINANCIALS [Retail Sales Weekly] Store Update and Data Load" -j "02-00 Manual - Retail Sales Weekly - Forecast Load" -c CurrYear=%CurrYear%>>%LogFileName%
  call .\etlclient -p "[04-03]-PAS-FINANCIALS [Retail Sales Daily] Store Update and Data Load" -j "01-02 Nightly - Retail Sales Daily Cube Forecast Load" -c CurrYear=%CurrYear%>>%LogFileName%
)
...
IF %ERRORLEVEL% EQU 0 (
  SET Forecast_Load_ReturnCode=2
)
IF %Forecast_Load_ReturnCode% NEQ 0 goto PasFinError

:: Now look for warnings  
type %LogFileName%|findstr /I /c:"Completed with Warnings"
IF %ERRORLEVEL% EQU 0 (
  SET Forecast_Load_ReturnCode=1
)
IF %Forecast_Load_ReturnCode% NEQ 0 goto PasFinError

:: If no errors with finance job found run the retail job
IF %Forecast_Load_ReturnCode% EQU 0 goto :eof

:PasFinError
:: There was an error in the Financials Load Test Job
set EmailReturnCode=%Forecast_Load_ReturnCode%
for /f "tokens=*" %%i in ('time /t') do set Time_now=%%i
for /f "tokens=*" %%i in ('date /t') do set Date_now=%%i
...
IF %EmailReturnCode% EQU 0 (
  set SubjectText="Jedox Retail Weekly Cube Forecast Load was Successful"
)
IF %EmailReturnCode% EQU 1 (
  set SubjectText="Jedox Retail Weekly Cube Forecast Load Completed with Warnings"
)
IF %EmailReturnCode% EQU 2 (
  set SubjectText="Jedox Retail Weekly Cube Forecast Load Failed"
)
