...
WHERE RetailCustomers.[Signup Store Code] = 'BAOLIDCO' 
...
WHERE FactRetailSales.SaleDate >= DATEADD(MONTH, -12, CAST(GETDATE() AS DATE)) 
  AND MatWarehouse.WarehouseCode = 'BAOLIDCO' 
  AND RetailCustomers.[Email Address] IS NOT NULL 
GROUP BY RetailCustomers.[Person Id] 
  ...
  --select * from MatWarehouse where WarehouseName like '%lidcombe%' -- ID 263 Code BAOLIDCO