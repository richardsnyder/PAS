...
IF %DebugFlag% EQU 0 (
  ...
  call .\etlclient -p "[02-00]-PAS-FINANCIALS [Retail] Store Update and Data Load" -j "[01-01]-load[Dim]:Store" >>%LogFileName%
...
IF %PasFin_TestJob_ReturnCode% EQU 0 goto CashFlowCubLoad
...
if %DebugFlag% EQU 1 (
  ...
  echo send-mailmessage -from ^"JedoxAdmin@pasco.com.au^" -to ^"pasaccountants@pasco.com.au^" -subject %SubjectText% -body $body -Attachment "%ret%" -priority High -dno onFailure -smtpServer mtwrly01.onepas.local >>%OutFile%
)
