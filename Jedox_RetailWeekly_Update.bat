...
if exist RetailWeeklyBatch*.log (
  del RetailWeeklyBatch*.log
)
...
set LogFileName="%ret%\RetailWeeklyBatch-%DateN%_%timen%.log"
...
IF %DebugFlag% EQU 0 (
  ...
  call .\etlclient -p "[04-03]-PAS-FINANCIALS [Retail Sales Daily] Store Update and Data Load" -j "01-01 Nightly - Retail Sales Daily Cube Actual Load" -c PrevYear=%PrevYear% CurrYear=%CurrYear% >>%LogFileName%
)
...
IF %ERRORLEVEL% EQU 0 (
  SET WeeklyRtlSalesActual_ReturnCode=1
)
...
IF %ERRORLEVEL% EQU 0 (
  SET WeeklyRtlSalesActual_ReturnCode=2
)
IF %WeeklyRtlSalesActual_ReturnCode% NEQ 0 goto PasWklyRtlSalesActualError
...
IF %WeeklyRtlSalesActual_ReturnCode% EQU 0 goto SendEmail
...
IF %EmailReturnCode% EQU 0 (
  set SubjectText="Jedox Retail Sales Weekly Cube Update was Successful"
)
IF %EmailReturnCode% EQU 1 (
  set SubjectText="Jedox Retail Sales Weekly Cube Update Completed with Warnings"
)
IF %EmailReturnCode% EQU 2 (
  set SubjectText="Jedox Retail Sales Weekly Cube Update Failed"
)
