...
if exist TargetLoadBatch*.log (
  del TargetLoadBatch*.log
)
...
set LogFileName="%ret%\TargetLoadBatch-%DateN%_%timen%.log"
...
c:
cd "C:\Program Files\Jedox\Jedox Suite\tomcat\client"
echo ***  Start of Jedox Retail Weekly Target Load Batch: %Date_now% %Time_now%  ***  >> %LogFileName%
...
IF %DebugFlag% EQU 0 (
  call .\etlclient -p "[04-01]-PAS-FINANCIALS [Retail Sales Weekly] Store Update and Data Load" -j "04-00 Manual - Retail Sales Weekly - Target Load" -c CurrYear=%CurrYear%>>%LogFileName%
)
...
IF %Payroll_Load_ReturnCode% EQU 0 goto :eof
