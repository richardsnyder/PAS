SELECT DISTINCT CAST(MatProduct.StyleColourCode AS NVARCHAR(50)) AS StyleColourId 
  ...
  , CAST(dbo.FuncGetTaxExclusiveAmount(COALESCE (NULLIF (WholesaleAudPrice.CurrentPrice, 0)
  , BackupWholesaleAudPrice.CurrentPrice) ,COALESCE (NULLIF (WholesaleAudPriceScheme.TaxRate, 0) , BackupWholesaleAudPriceScheme.TaxRate)) AS DECIMAL(18 ,4)) AS Wholesale_Price 
  ,CAST(FactProductCost.Cost / 
      CASE 
      ...
      END AS DECIMAL(18,4)) AS Cost_Price 
        ...
        , CAST(dbo.FuncGetTaxExclusiveAmount(COALESCE (NULLIF (RRPAudPrice.CurrentPrice, 0)
  , BackupRRPAudPrice.CurrentPrice) ,COALESCE (NULLIF (RRPAudPriceScheme.TaxRate, 0) , BackupRRPAudPriceScheme.TaxRate)) AS DECIMAL(18 ,4)) AS RRP_Price 
...
WHERE 1=1 
 ...
 AND MatWarehouse.WarehouseCode = 'DWMAIN' 
  ...
  AND MatProduct.BusinessDivisionCode IN ('18', '10', '26','16')
  AND ComponentGroupCode = 'FG'
  AND MatProduct.IsActive = 1
  AND MatProduct.ColourId != 1
  AND DimDate.CalendarDate >= (Select distinct  RetailYearStart from DimDate where CalendarDate = CAST(GETDATE() AS DATE))
  AND DimDate.CalendarDate <=  (Select distinct  RetailYearEnd from DimDate where CalendarDate = CAST(GETDATE() AS DATE))
